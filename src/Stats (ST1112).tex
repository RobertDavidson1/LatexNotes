...
\begin{conceptbox}{Stages in Hypothesis Testing}{}
    ...
    Identify the sampling distribution of the test statistic (usually via the Central Limit Theorem or a t-distribution):
        ...
        For smaller nn (and unknown $\sigma$), use a t-distribution with $n-1$ degrees of freedom, assuming approximate normality of the population.

        Decide whether the observed test statistic would be rare or common if $H_0$ were true:

        p-value approach: Probability of obtaining a result at least as extreme as the actual sample result, given $H_0$ is true.

        Rejection region approach: Compare the test statistic to a critical value derived from the chosen distribution and significance level $\alpha$
...
\end{enumerate}
\end{examplebox}
\begin{conceptbox}{Hypothesis Test Using p-values}{}
    Instead of using a formal "rejection", many prefer to use the \textbf{p-value}:
    \begin{itemize}
        \item \textbf{Compute} $T_0$ from the data \\
        \item \textbf{Compute p-value} = probability under $H_$ of observing a test statistic or more extreme than $T_0$
        \item \textbf{Compare p-value to $\alpha$}:
        \begin{itemize}
            \item p-value $\leq \alpha$, reject $H_0$
            \item p-value $> \alpha$ do not reject $H_0$
        \end{itemize}
    \end{itemize}
\end{conceptbox}
