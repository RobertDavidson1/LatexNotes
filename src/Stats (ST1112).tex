...
\subsubsection{CI with large $n$, $\sigma$ uknown but replaced with $s$}
...
where $Z_{\alpha/2}$ is the z-score for the chosen confidence level (e.g. $Z_{0.025} = 1.96$ for a 95\% CI). If $\sigma$ is not known but $n$ is large, we replace $\sigma$ with the sample standard deviation $s$:
