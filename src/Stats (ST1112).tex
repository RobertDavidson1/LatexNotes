...
\begin{conceptbox}{Underlying Distributions}{}
    ...
    \end{itemize}
\end{conceptbox}
\begin{conceptbox}{Rejection Region (Classical Approach)}{}
    Depends on the alternative hypothesis: 
    \begin{itemize}
        \item \textbf{Left Tailed (One sided) ($\mu < \mu_0$)}: reject if $T_0 < -t_{\alpha, df}$.
        \item \textbf{Right Tailed (One sided) ($\mu > \mu_0$)}: reject if $T_0 > t_{\alpha, df}$.
        \item \textbf{Two-Sided ($\mu \neq \mu_0$)}: reject if $T_0 < -t_{\alpha/2, df}$ or $T_0 > t_{\alpha/2, df}$.
    \end{itemize}
\end{conceptbox}
...
\end{conceptbox}
\noindent\textbf{Important Clarification}: \\
