...
\begin{tikzpicture}[
    ...
    % Fill background
    \fill[blue!15] (-3.5,0) rectangle (3.5,0.45);
    
    % Fill left tail area (2.5%)
    \fill[yellow!80] (-3.5,0) -- plot[domain=-3.5:-\zcrit] (\x, {0.4*exp(-(\x)^2/2)}) -- (-\zcrit,0) -- cycle;
    
    % Fill right tail area (2.5%)
    \fill[yellow!80] (3.5,0) -- plot[domain=3.5:\zcrit] (\x, {0.4*exp(-(\x)^2/2)}) -- (\zcrit,0) -- cycle;
    
    % Draw the normal distribution curve
    \draw[thick, yellow!95!black] plot[domain=-3.5:3.5] (\x, {0.4*exp(-(\x)^2/2)});
    
    % Draw horizontal axis
    \draw[->, thick] (-3.7,0) -- (3.7,0) node[right] {};
    
    % Draw vertical axis
    \draw[->, thick] (0,0) -- (0,0.45) node[above] {};
    ...
    % Add labels
    \node[above left] at (-2.5,0.1) {$\text{P}(z < -z_{0.025}) = 0.025$};
    \node[above] at (0,0.25) {$\text{P}(-z_{0.025} < z < z_{0.025}) = 0.95$};
    \node[above right] at (2.5,0.1) {$\text{P}(z > z_{0.025}) = 0.025$};
    
    % Title
    \node[above] at (0,0.43) {\Large \textbf{Standard Normal Distribution ($z$)}};
    
    % Question marks
    \node[below, green!60!black] at (-\zcrit,-0.1) {?};
    \node[below, green!60!black] at (\zcrit,-0.1) {?};
    
\end{tikzpicture}


\begin{definitionbox}{Crritical Values}{}
    The \textbf{critical value} is a z-score that corresponds to the desired confidence level. \\
    
    For example, for a 95\% confidence level, the critical value is $Z_{\alpha/2} = 1.96$ (where $\alpha = 0.05$).\\
    This means that 95\% of the area under the normal curve lies within $1.96$ standard deviations of the mean.
\end{definitionbox}
