    ...
    \begin{minipage}{0.45\linewidth}
        ...
        \begin{tikzpicture}[scale = 0.5]
            \begin{axis}[
                ...
                samples=100,
                xlabel={$$},
                title={\Huge Skewed Data (Log-normal)},
                width=\linewidth,
                axis x line=bottom,
                axis y line=left,
                ytick=\empty,
            ...
            \addplot [custom_blue, thick] {1/(x*0.5*sqrt(2*pi)) * exp(-(ln(x))^2/(2*0.5^2))};
            \end{axis}
        \end{tikzpicture}
    \end{minipage}
    \hfill
    \begin{minipage}{0.45\linewidth}
        ...
        \begin{tikzpicture}[scale = 0.5]
            \begin{axis}[
                ...
                samples=100,
                xlabel={$$},
                title={\Huge Heavy Tailed Data (Cauchy)},
                width=\linewidth,
                axis x line=bottom,
                axis y line=left,
                ytick=\empty,
            ...
            \addplot [custom_red, thick] {1/(pi*(1+x^2))};
            \end{axis}
        \end{tikzpicture}
    \end{minipage}
\end{figure}
...
\begin{definitionbox}{Proportion}{}
    A \textbf{proportion} is the ratio of the number of times a particular outcomes occurs (successes) to the total number of trials (or observations). Formally if $X$ represents the number of successes and $n$ is the total number of trials, then the proportion $p$ is given by:
    $$p = \frac{X}{n}$$
...
If $X \sim B(n, \pi)$, then $X$ is the number of successes in $n$ independent Bernoulli trials, each probability of success $\pi$.\\[2ex]
The \textbf{sample proportion} $p = \frac{X}{n}$ is the observed successes in the sample. 
\subsubsection{Normal Approximation of $p$}
For large enough $n$, so that $n\pi \geq 5$ and $n(1 - \pi) \geq 5$, the sampling distribution of the sample proportion $p$ can be approximated by a normal distribution:
$$X \simeq N\left(n\pi, n\pi(1- \pi)\right) \Rightarrow p = \frac{X}{n} \simeq N\left(\pi, \frac{\pi(1 - \pi)}{n}\right)$$
\end{document}
