...
\begin{tikzpicture}[
    ...
    \fill[custom_blue] (-3.5,0) -- plot[domain=-3.5:-\zcrit] (\x, {0.4*exp(-(\x)^2/2)}) -- (-\zcrit,0) -- cycle;
    
    % Fill right tail area (2.5%)
    \fill[custom_blue] (3.5,0) -- plot[domain=3.5:\zcrit] (\x, {0.4*exp(-(\x)^2/2)}) -- (\zcrit,0) -- cycle;
    
    % Draw the normal distribution curve
    \draw[thick, custom_blue] plot[domain=-3.5:3.5] (\x, {0.4*exp(-(\x)^2/2)});
    ...
    \draw[<->] (-\zcrit, -0.1) -- (\zcrit, -0.1) node[midway, below] {$95\%$};
    \draw[<->] (-\zcrit, -0.1) -- (-3.7, -0.1) node[midway, below] {$2.5\%$};
    \draw[<->] (\zcrit, -0.1) -- (3.7, -0.1) node[midway, below] {$2.5\%$};
