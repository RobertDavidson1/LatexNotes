...
\section{Concurrency}
\begin{definitionbox}{}{}
    \textbf{A cooperating process} is one that can affect or be affected by another process that is exeucting on the system.
\end{definitionbox}
\noindent \textbf{Threads} are prime examples of this: we can think of them as a single
process with multiple points of execution. They share program code and,
crucially, data. \\
A \textbf{Race Conidition}  is one where the result depends on the order in which instructions are executed.
For a single thread process.
