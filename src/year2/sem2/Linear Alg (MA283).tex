...
\begin{itemize}
  ...
  \item The system may be \textbf{consistent} which occurs if:
        ...
        \end{itemize}
\end{itemize}
\subsection{Gaussian Elimination and Matrix Algebra}
...
$$I_3 + 4E_{1,2} = \begin{bmatrix}
    ...
    0 & 0 & 1
  \end{bmatrix}$$
Performing a row operation on $A$ is the same as multiplying $A$ by an appropriate matrix $E$ on the left \\[2ex]
\textbf{Multiplying a row by a non zero scalar}
\begin{itemize}
  \item When you multiply row $i$ of $A$, by some scalar $\alpha \neq 0$, all other rows remain unchanged, and the $i$th row is scaled by $\alpha$.
  \item The Identity matrix $I_m$ leaved $A$ unchanged if we multiply $I_mA$
  \item To scale row $i$ by $\alpha$,you want the $(i,i)$-entry of this matrix to be $\alpha$, so the $i$th row of $A$ is scaled by $\alpha$.
  \item Hence $I_m + (\alpha - 1)E_{i,i}$ causes the $i$th row of $A$ to be scaled by $\alpha$.
\end{itemize}
That is what the matrix
$$I_m + (\alpha - 1)E_{i,i}$$
does to $A$:
Observe that $I_m$ has all diagonal entries $= 1$. Adding $(\alpha-1)E_{i,i}$  makes that diagonal entry become $\alpha$ and all other diagonal entries remain $1$. Example:
$$I_3 + 4_E{2,2} = \begin{bmatrix}
    1 & 0 & 0 \\
    0 & 5 & 0 \\
    0 & 0 & 1
  \end{bmatrix}
  ...
  \begin{bmatrix}
    ...
    4 & 5 & 6 \\
    7 & 8 & 9
  \end{bmatrix} \quad \text{then} \quad (I_3 + 4E_{2,2})A =
  \begin{bmatrix}
    1  & 2  & 3  \\
    20 & 25 & 30 \\
    7  & 8  & 9
  \end{bmatrix}$$
\textbf{Switching two rows}
\begin{itemize}
  \item Consider the effect of switching two ros in a matrix. The swapping matrix $S$ must do the following:
        \begin{itemize}
          \item Take row $i$ of $A$ and put it in row $k$ of $SA$
          \item Take row $k$ of $A$ and put it in row $i$ of $SA$
          \item Leave all other rows unchanged
        \end{itemize}
  \item One way to build this swapping matrix is to start with $I_m$ and then tinker with the rows/columns so that the $i$th rwo picks out the old $k$th row and the $k$th row picks out the old $i$th row.
\end{itemize}
The matrix:
$$
  S = I_m + E_{i,k} + E_{k,i} - E_{i,i} - E_{k,k}
$$ does this. Observe that:
\begin{itemize}
  \item Normally $I_m$ has 1's on the diagonal $(j,j)$
  \item We subtract the 1's positions $(i,i)$ and $(k,k)$, so those spots become 0.
  \item We add 1's in the positions $(i,k)$ and $(k,i)$, so those spots become 1.
\end{itemize}
$$
  S = I_3 + E_{1,3} + E_{3,1} - E_{1,1} - E_{3,3}
  = \begin{bmatrix}
    0 & 0 & 1 \\
    0 & 1 & 0 \\
    1 & 0 & 0
...
\begin{itemize}
  \item Suppose you want to replace row $k$ by with row $k + \alpha \times $ row $i$.
  \item Starting with the identity matrix $I_m$, multiplying on the left by $I_m$ would leave $A$ unchanged.
  \item To add $\alpha \times$ row $i$ to row $k$, we want the multiplication to say:
        \begin{itemize}
          \item Keep $j$ unchanged, for all $j \neq k$
          \item Replace row $k$ with itself plus $\alpha$ times row $i$
        \end{itemize}
\end{itemize}
The matrix:
$$I_m + \alpha E_{k,i}$$
does this. Observe that:
\begin{itemize}
  \item Row $j$ of $I_m + \alpha E_{k,i}$ is just the standard basis row whenever $j \neq k$. So row $j$ of $A$ is unchanged.
  \item Row $k$ of $I_m + \alpha E_{k,i}$ is
        $$\text{Row } k \text{ of } I_m + \alpha \text{Row } i \text{ of } I_m$$
\end{itemize}
\end{document}
