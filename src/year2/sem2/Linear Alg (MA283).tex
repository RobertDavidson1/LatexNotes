...
A $m \times n$ matrix over a field $\mathbb{F}$ is an array of $m$ rows, and $n$ columns with elements $\in \mathbb{F}$. Two matrices can be added if they have the same size; in this case their sum is obtained by adding the corresponding entries. \\[2ex]
The \textbf{zero matrix} is a matrix with all entries equal to 0. It is the \textbf{identity element} for addition of matrices - adding it to any matrix $A$ gives $A$ back. \\[2ex]
A matrix can be multiplied by a scalar by multiplying each entry of the matrix by the scalar. \\[2ex]
With these operations of addition and scalar multiplication, the set of $m \times n$ matrices of a field $\mathbb{F}$ is a \textbf{vector space} over $\mathbb{F}$. We use the notation:
$$M_{m \times n}(\mathbb{F})$$
for the vector space of all $m \times n$ matrices over $\mathbb{F}$. When $m = n$, we abbreviate this to:
$$M_n(\mathbb{F})$$

\noindent \textbf{Example:}  $M_{2 \times 3}(\mathbb{R})$:
$$
  2
  \begin{pmatrix}
    1 & 0  & -1 \\
  ...
  \end{pmatrix}=
  \begin{pmatrix}
    2(1)-3(2) & 2(0)-3(4)  & 2(-1)-3(-1) \\
    2(2)-3(0) & 2(-5)-3(1) & 2(-3)-3(-3)
  \end{pmatrix}=
  \begin{pmatrix}
    -4 & -12 & 1 \\
    4  & -13 & 3
...
$$
\begin{definitionbox}{}{}
  Suppose that $\mathbf{v_1}, \mathbf{v_2}, \ldots, \mathbf{v_k}$ are elements of a vector space $\mathcal{V}$ over a field $\mathbb{F}$. A $\mathcal{F}$-linear combination of these vectors is an element of $\mathcal{V}$ of the form:
  $$\alpha_1 \mathbf{v_1} + \alpha_2 \mathbf{v_2} + \ldots + \alpha_k \mathbf{v_k}$$
  where $\alpha_i \in \mathbb{F}$ for $i = 1, 2, \ldots, k$ are the \textbf{coefficients} of the linear combination.
\end{definitionbox}
\begin{definitionbox}{}{}
  Let $A$ be a $m \times n$ matrix, and let $v$ be a column vector with $n$ entries. Then the matrix vector product $Av$ is obtained by taking the linear combination of the columns of $A$ whose \textbf{coefficients are the entries of $v$}. The result is a column vector with $m$ entries.
  $$\begin{bmatrix}
      2 & 4 & -1 \\
      0 & 1 & -3
    \end{bmatrix}
    ...
    \end{bmatrix}
  $$
  Note, the product $Av$ is only defined when the number of columns of $A$ is equal to the number of entries in $v$.
\end{definitionbox}
\begin{definitionbox}{}{}
  Let $A$ and be $B$ be matrices of size $m \times p$ and $p \times n$ respectively. Write $v_1, v_2, \ldots, v_p$ for the columns of $B$. Then the matrix product $AB$  is the $m \times n$ matrix whose columns are $Av_1, Av_2, \ldots, Av_n$.
\end{definitionbox}
For a field $\mathbb{F}$, we write $\mathbb{F}^p$ is the vector space for all vector with $p$ entries from $\mathbb{F}$. \\[2ex]
Sometimes we need to specify whether we mean a row vector or column vectors, but for now we'll cheat and allow every ordered list of $p$ elements of $\mathbb{F}$ to be considered as an element of $\mathbb{F}^p$. \\[2ex]
