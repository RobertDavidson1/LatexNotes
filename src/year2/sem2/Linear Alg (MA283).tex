...
\pagebreak
\subsection{idk}
Elementary Row Operations can be used to find the inverse of a square matrix. Suppose $\mathcal{A} \in M_n{\mathbb{F}}$, for some field $\mathbb{F}$. If $\mathcal{A}$, let $v_1, v_2, \ldots, v_n$ be the columns of $\mathcal{A}$. Then:
$$\left.\mathcal{AA}^{-1}=\mathcal{A}\left[
    \begin{array}
      {cccc}| & |   & \ldots & |   \\
      v_1     & v_2 & \ldots & v_n \\
      |       & |   & \ldots & |
    \end{array}\right.\right]=\mathrm{A}
  \begin{bmatrix}
    |    & |    & \ldots & |    \\
    Av_1 & Av_2 & \ldots & Av_n \\
    |    & |    & \ldots & |
  \end{bmatrix}=\mathrm{I_n}.$$
\end{document}