...
\end{definitionbox}
\begin{definitionbox}{}{}
  Let $A$ and be $B$ be matrices of size $m \times p$ and $p \times n$ repeatively. Write $v_1, v_2, \ldots, v_p$ for the columns of $B$. Then the matrix product $AB$  is the $m \times n$ matrix whose columns are $Av_1, Av_2, \ldots, Av_n$.
\end{definitionbox}
For a field $\mathbb{F}$, we write $\mathbb{F}^p$ is the vector space for all vector with $p$ entries from $\mathbb{F}$
\end{document}