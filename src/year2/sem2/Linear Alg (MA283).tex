...
To find all $\mathbf{v}_i$ simultaneously, we form the $n \times 2n$ augmented matrix $[A \;|\; I_n]$. We then apply row operations until the left portion of the augmented matrix is reduced to the identity matrix ($A$'s RREF). If $A$ is invertible, after applying these row operations, the matrix on the left becomes $I_n$, and the matrix on the right becomes $A^{-1}$. Symbolically,
\[
