...
\begin{figure}[htbp]
  ...
  \begin{subfigure}{0.45\textwidth}
    ...
    \begin{tikzpicture}[scale=0.8]
      ...
      \draw[->, thick, dashed, custom_red] (0,0) -- (2,0);
      \draw[->, thick, dashed, custom_red] (0,0) -- (-2,0);
      \draw[->, thick, dashed, custom_red] (0,0) -- (0,2);
      \draw[->, thick, dashed, custom_red] (0,0) -- (0,-2);

      \draw[->, thick, custom_red] (0,0) -- (1.414,1.414);
      \draw[->, thick, custom_red] (0,0) -- (-1.414,1.414);
      \draw[->, thick, custom_red] (0,0) -- (1.414,-1.414);
      \draw[->, thick, dashed, custom_red] (0,0) -- (-1.414,-1.414);



      % Label contour lines
      \node[above, custom_blue] at (0,2) {contour lines};
    \end{tikzpicture}
    \caption{Contour Plot with Gradient Vectors}
  \end{subfigure}
  \hfill
  \begin{subfigure}{0.45\textwidth}
    \centering
    \begin{tikzpicture}[scale=0.8]
      \begin{axis}[
          ...
          ztick=\empty,
          axis equal,
          colormap={paraboloid}{color(0)=(custom_blue!5) color(1)=(custom_blue)},
        ]
...
\subsection{Gradient as Normal to a Surface}
\begin{center}
  \begin{tikzpicture}
    \begin{axis}[
        ...
        ytick=\empty,
        ztick=\empty,
        colormap={bluered}{color(0)=(custom_blue!30) color(1)=(custom_red!50)}
      ]
      % Plot the surface z = 1 - x^2/4 - y^2/4
      \addplot3[
        surf,
        domain=-2:2,
        domain y=-2:2,
      ...
      % Add normal vectors (gradients) at various points
      \addplot3[
        ->,
        thick,
        custom_blue,
      ...
      % Label the surface
      \node[right] at (axis cs:2,0,0) {$S: z = 1 - \frac{x^2}{4} - \frac{y^2}{4}$};

      % Label the normal vectors
      \node[above] at (axis cs:0,0,1.1) {$\nabla\phi = \text{normal to }S$};

    \end{axis}
  \end{tikzpicture}
\end{center}

\begin{theorembox}[Gradient Normal to Level Surface]
  If $S$ is a level surface defined by $\phi(x,y,z) = C$ where $C$ is a constant, and if $\nabla\phi \neq 0$ at a point $P$ on $S$, then $\nabla\phi$ at $P$ is normal to the surface $S$ at $P$.
\end{theorembox}
