...
\begin{center}
  \begin{tikzpicture}[scale=1]
    \node at (0,3.5) {$P$};
    \draw[->] (0,0) -- (0,3) node[midway, left] {$\vec{r}_0$};

    \node at (3.5, 1.5) [right] {$Q(s)$};
    \draw[->] (0,3) -- (3,1.5) node[midway, above] {$s$};

    \draw[->] (0,0) -- (3,1.5) node[midway, below] {$\vec{r(s)}$};
    \node at (0,0) [below] {$O$};
  \end{tikzpicture}
\end{center}
...
\begin{tikzpicture}[scale=1.5]
  ...
  \fill[black] (P) circle (2pt) node[below left] {$P(\vec{r}_0)$};

  % Point Q
  \coordinate (Q) at (1,1,1);
  \fill[black] (Q) circle (2pt) node[above right] {$Q(s)$};

  % Line segment L
  \draw[thick] (P) -- (Q) node[midway, above] {$L$};

  % Direction vector n
  \draw[->, thick, blue] (P) -- (Q) node[midway, above right] {$\hat{\mathbf{n}}$};

  % Position vector r0
  \draw[->, thick, red] (0,0,0) -- (P) node[midway, below left] {$\vec{r}_0$};

  % Position vector r0 + s*n
  \draw[->, thick, green] (0,0,0) -- (Q) node[midway, above left] {$\vec{r}_0 + s\hat{\mathbf{n}}$};

  % Surface representation (simplified plane)
  \draw[fill=gray!20, opacity=0.5] (-1.5,-1.5,-0.5) -- (1.5,-1.5,1.5) -- (1.5,1.5,1.5) -- (-1.5,1.5,-0.5) -- cycle;

  % Gradient vector at P
  \draw[->, thick, orange] (P) -- (0.5, 0.2, 0.8) node[above right] {$\nabla \phi$};

  % Label the distance s
  \draw[dashed] (P) -- (Q);
  \node at (0.5, 0.5, 0.5) [above right] {$s$};

  % Definition Box
  \node[anchor=north west, draw, rounded corners, fill=yellow!20, inner sep=5pt] (defbox) at (-2,1.5) {
    \begin{minipage}{4cm}
      \textbf{Definition:} The gradient of $\phi(x,y,z)$ is:
      \[ \nabla \phi = \frac{\partial \phi}{\partial x} \hat{i} + \frac{\partial \phi}{\partial y} \hat{j} + \frac{\partial \phi}{\partial z} \hat{k} \]
    \end{minipage}
  };

  % Directional Derivative Box
  \node[anchor=south west, draw, rounded corners, fill=cyan!20, inner sep=5pt] (dirbox) at (-2,-1.5) {
    \begin{minipage}{4cm}
