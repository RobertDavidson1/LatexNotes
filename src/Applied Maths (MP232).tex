...
Let $\delta_{\epsilon}(t)$ be a some function which is very large over a short intercal, $t \in [-\epsilon,\epsilon]$, with $\epsilon > 0$, but is $0$ elsewhere.
$$
...
Futhermore, we may define the integral of the product of the Dirac Delta Function with a function $f(t)$ as:
$$\int_{-\infty}^{\infty} \delta(t-t_0)f(t) \; dt = \lim_{\varepsilon \to 0+}\int_{-\infty}^{\infty} \delta_{\varepsilon} (t-t_0)f(t)\;dt = f(t_0)$$
Thus, the Laplace Transform of the Dirac Delta Function is:
$$L\{\delta(t-t_0)\} = \int_{0}^{\infty} e^{-st}\delta(t - t_0) \; dt = \int_{-infty}^{\infty} e^{-st}\delta(t-t_0) \; dt = e^{-st_0} \quad \text{for} \;
\; t_0 > 0$$
