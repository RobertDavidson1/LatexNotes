...
\begin{examplebox}[Determine and sketch the image under the map $w = e^z$, $\{z \in \mathbb{C} : \pi / 4 \leq \text{Im}(z) \leq \pi /2\}$ ]
    ...
    \begin{center}
        \begin{tikzpicture}
            ...
            \draw[custom_blue, thick] ({\1*cos(45)},{\1*sin(45)}) arc (45:90:\radius);
            \draw[custom_blue, thick] ({\radius*cos(45)},{\radius*sin(45)}) arc (45:90:\radius);

            \fill[custom_blue!20, opacity=0.3] (0,0) -- ({\radius*cos(45)},{\radius*sin(45)})
...
\begin{tikzpicture}[scale=3]
    % Draw axes
    \draw[->] (-0.1,0) -- (2.5,0) node[right] {$\Re w$};
    \draw[->] (0,-0.1) -- (0,2.5) node[above] {$\Im w$};

    % Define the radius to display (this is arbitrary since |w|>0 in general)
    \def\radius{2}

    % Draw the two boundary rays: angle = pi/4 and pi/2
    \draw[blue, thick] (0,0) -- ({\radius*cos(45)},{\radius*sin(45)}) node[midway, below right] {$\pi/4$};
    \draw[blue, thick] (0,0) -- ({\radius*cos(90)},{\radius*sin(90)}) node[midway, left] {$\pi/2$};

    % Draw the circular arc connecting these two rays at radius = 2


    % Optionally, fill the sector region
    \fill[blue!20, opacity=0.3] (0,0) -- ({\radius*cos(45)},{\radius*sin(45)})
    arc (45:90:\radius) -- cycle;

    % Label the region (optional)
    \node at (1.5,1.5) {$w=e^z$ Sector};
\end{tikzpicture}
