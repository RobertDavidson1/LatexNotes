...
\begin{conceptbox}[How we implement shared buffers]
    ...
    \end{itemize}
\end{conceptbox}
\noindent The two standard forms of IPC are \textbf{Direct} and \textbf{Indirect} communication. Furthermore, IPC may also be \textbf{Symmetric} or \textbf{Asymmetric}.
\subsubsection{Direct Communication}
\begin{itemize}
    \item Processes must nomiate the other process they want to communicate with
    \item Pairs of communicating processes must know eachothers ID's in order to send and recieve messages
    \item A link is established automitcally.
    \item There are exactly two processes in the link
    \item There can be at most one link between two processes
\end{itemize}
