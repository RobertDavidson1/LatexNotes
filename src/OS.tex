...
\begin{definitionbox}[Process Communication Types]
...
\begin{conceptbox}[Why use threads over processes]
    ...
    \end{itemize}
\end{conceptbox}
\subsubsection{Two types of threads - User and Kernel}
\begin{center}
...
\end{center}
\subsection{Constasting subprocs and threads}
These programs will have a globl int \texttt{GlobalVar}, initalised as 0. First we'll see that subprocces that increment GlobalVar do not change the parents value.
\begin{lstlisting}[style=cStyle, caption={Subprocs and GlobalVar}]
...
\end{itemize}
\begin{lstlisting}[style=cStyle, caption={Threads and GlobalVar}]
    ...
    {
        val++;
        printf("\tThread with (PID=%d); val=%d\n",
            getpid(), val);
        return(NULL);
    }
    ...
    int main(void)
    {
        ...
        printf("START:\tParent with (PID=%d); val=%d\n",
            getpid(), val);
        ...
        printf("END:\tParent with (PID=%d); val=%d\n",
            getpid(), val);
        return 0;

        Output:
        "An example showing that a pthread shares its parent's memory
        START: Parent with (PID=25149); val=0
        Thread with (PID=25149); val=1
        END: Parent with (PID=25149); val=1"
    }
\end{lstlisting}
\section{Scheduling Processes}
\begin{definitionbox}
    \textbf{CPU Burst} : the amount of time the process uses the processor before it is no longer ready.
    \textbf{I/O Burst} : the amount of time the process is waiting for I/O to complete.
\end{definitionbox}

CPU scheduling is the basis of multipogrammed operating systems.
\begin{conceptbox}[Underling concepts of CPU scheduling]
    \begin{itemize}
        \item \textbf{Goal}: Maximum CPU utilization, there should be something running at any given time.
        \item \textbf{Based around}: CPU burst or I/O burst Cycle. A running process alternates between executing instructions (CPU burst) and waiting for I/O operations to complete (I/O burst).
        \item \textbf{Depends on}: the burst distribution, given a proc predominantly concerned with computation or I/O. Also a proc usually begins with a long CPU burst and alomst always ends with a CPU burst.
    \end{itemize}
\end{conceptbox}
...
\begin{itemize}
    \item Switches from running $\to$ waiting state (non preemptive)
    \item Terminates (non preemptive)
    \item Switches from running $\to$ ready state (preemptive)
    \item Switches from waiting $\to$ ready state (preemptive)
\end{itemize}
If a scheduler uses only non-premptive methods, once a process is allocated to the CPU it will reamin there until it terminates or changes states. \\[2ex]
...
\begin{itemize}
    \item First Come First Served (FCFS) - the process that arrives first is executed first. This is a non-preemptive algorithm.
    \item Shortest Job First (SJF) - the process with the shortest CPU burst time is executed first. This is a non-preemptive algorithm.
    \item Shortest Time To Completion (STC) - the process with the shortest time to completion is executed first. This is a preemptive algorithm.
    \item Round Robin (RR) - each process is given a fixed time slice (quantum) to execute. If the process does not finish within the time slice, it is preempted and the next process is executed. This is a preemptive algorithm.
\end{itemize}
...
\begin{itemize}
    \item Turnaround time - the time that elapses between when proc arrives in system and when it is completed.
    \item Wait time - time between when a proc arrives and when it comples, that it is spent doing nothing.
    \item Reponse time - time between when proc arrives and when it first starts executing.
\end{itemize}
